...
from keras.layers import AveragePooling2D, InputLayer, Flatten, MaxPool2D, Dropout
    ...
    def call(self, inputs):
        ...
        for stack in range(3):
            for res_block in range(num_res_blocks):
                ...
                y = Resnet_layer(num_filters=num_filters, strides=strides)(inputs=x)
                y = Resnet_layer(num_filters=num_filters, activation=None)(inputs=y)
                if stack > 0 and res_block == 0:  # first layer but not first stack
        ...
        outputs = Dense(self.num_classes, activation=None, kernel_initializer="he_normal")(y)
